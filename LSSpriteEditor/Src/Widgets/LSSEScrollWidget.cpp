...
#include "LSSEScrollWidget.h"
#include "../LSSESpriteEditor.h"


// == Various constructors.
LSE_CALLCTOR CScrollWidget::CScrollWidget( QWidget * _pwParent ) :
	QScrollArea( _pwParent ) {
}
 ...
 */
void CScrollWidget::scrollContentsBy( int _iDx, int _iDy ) {
	QScrollArea::scrollContentsBy( _iDx, _iDy );
	if ( LSSpriteEditor::GetMainWindow() ) {
		LSSpriteEditor::GetMainWindow()->RedrawSideImages();
	}
}
 ...
 */
bool CScrollWidget::viewportEvent( QEvent * _peEvent ) {
	if ( LSSpriteEditor::GetMainWindow() ) {
		LSSpriteEditor::GetMainWindow()->RedrawSideImages();
	}
	return QScrollArea::viewportEvent( _peEvent );
}