/**
 * Copyright L. Spiro 2014
 * All rights reserved.
 *
 * Written by:	Shawn (L. Spiro) Wilcoxen
 *
 * This code may not be sold or traded for any personal gain without express written consent.  You may use
 *	this code in your own projects and modify it to suit your needs as long as this disclaimer remains intact.
 *	You may not take credit for having written this code.
 *
 *
 * Description: A template for any type of 2D vector.
 */


#ifndef __LSM_VECTOR2BASE_H__
#define __LSM_VECTOR2BASE_H__

#include "../LSMMathLib.h"

namespace lsm {

	/**
	 * Class CVector2Base
	 * \brief A template for any type of 2D vector.
	 *
	 * Description: A template for any type of 2D vector.
	 */
	template <typename _tType, typename _tInher>
	class CVector2Base {
		// All is public.  This class has no secrets.
	public :
		// == Various constructors.
		LSE_CALLCTOR								CVector2Base() {
		}
		LSE_CALLCTOR								CVector2Base( LSE_UNALIGNED const _tInher &_v2bOther ) :
			x( _v2bOther.x ),
			y( _v2bOther.y ) {
		}
		LSE_CALLCTOR								CVector2Base( _tType _fX, _tType _fY ) :
			x( _fX ),
			y( _fY ) {
		}
		LSE_CALLCTOR								CVector2Base( _tType * _pfValues ) :
			x( _pfValues[0] ),
			y( _pfValues[1] ) {
		}

		// == Operators.
		/**
		 * Array access.
		 *
		 * \param _ui32Index Index of the dimension to get.
		 * \return Returns the value at the given dimension.
		 */
		_tType LSE_FCALL							operator [] ( uint32_t _ui32Index ) const {
			return reinterpret_cast<const _tType *>(this)[_ui32Index];
		}

		/**
		 * Array access.
		 *
		 * \param _ui32Index Index of the dimension to get.
		 * \return Returns the value at the given dimension.
		 */
		_tType & LSE_FCALL							operator [] ( uint32_t _ui32Index ) {
			return reinterpret_cast<_tType *>(this)[_ui32Index];
		}

		/**
		 * Assignment.
		 *
		 * \param _v2bOther The vector to copy.
		 * \return Returns the copied vector.
		 */
		_tInher & LSE_FCALL							operator = ( const _tInher &_v2bOther ) {
			x = _v2bOther.x;
			y = _v2bOther.y;
			return (*static_cast<_tInher *>(this));
		}

		/**
		 * Unary -.
		 *
		 * \return Returns a copy of this vector with each axis negated.
		 */
		_tInher LSE_FCALL							operator - () const {
			return CVector2Base<_tType, _tInher>( -x, -y );
		}

		/**
		 * Binary addition.
		 *
		 * \param _v2bOther The vector to add to this one componentwise.
		 * \return Returns a new vector representing the addition.
		 */
		_tInher LSE_FCALL							operator + ( const _tInher &_v2bOther ) const {
			return _tInher( x + _v2bOther.x, y + _v2bOther.y );
		}

		/**
		 * Binary subtraction.
		 *
		 * \param _v2bOther The vector to subtract from this one componentwise.
		 * \return Returns a new vector representing the difference.
		 */
		_tInher LSE_FCALL							operator - ( const _tInher &_v2bOther ) const {
			return _tInher( x - _v2bOther.x, y - _v2bOther.y );
		}

		/**
		 * Binary multiplication.
		 *
		 * \param _tValue The amount by which to multiply each component in this vector.
		 * \return Returns a new vector representing the product.
		 */
		_tInher LSE_FCALL							operator * ( _tType _tValue ) const {
			return _tInher( x * _tValue, y * _tValue );
		}

		/**
		 * Binary division.
		 *
		 * \param _tValue The amount by which to divide each component in this vector.
		 * \return Returns a new vector representing the quotient.
		 */
		_tInher LSE_FCALL							operator / ( _tType _tValue ) const {
			return _tInher( x / _tValue, y / _tValue );
		}

		/**
		 * Adds a vector to this vector, storing the result in this vector.
		 *
		 * \param _v2bOther The vector to add to this vector.
		 * \return Returns this vector, after the addition operation has been applied.
		 */
		_tInher & LSE_FCALL							operator += ( const _tInher &_v2bOther ) {
			x += _v2bOther.x;
			y += _v2bOther.y;
			return (*static_cast<_tInher *>(this));
		}

		/**
		 * Subtract a vector from this vector, storing the result in this vector.
		 *
		 * \param _v2bOther The vector to subtract from this vector.
		 * \return Returns this vector, after the subtraction operation has been applied.
		 */
		_tInher & LSE_FCALL							operator -= ( const _tInher &_v2bOther ) {
			x -= _v2bOther.x;
			y -= _v2bOther.y;
			return (*static_cast<_tInher *>(this));
		}

		/**
		 * Multiply the vector by a given amount, storing the result in this vector.
		 *
		 * \param _tValue The amount by which to multiply each component in this vector.
		 * \return Returns this vector, after the muliplication operation has been applied.
		 */
		_tInher & LSE_FCALL							operator *= ( _tType _tValue ) {
			x *= _tValue;
			y *= _tValue;
			return (*static_cast<_tInher *>(this));
		}

		/**
		 * Divide the vector by a given amount, storing the result in this vector.
		 *
		 * \param _tValue The amount by which to divide each component in this vector.
		 * \return Returns this vector, after the division operation has been applied.
		 */
		_tInher & LSE_FCALL							operator /= ( _tType _tValue ) {
			x /= _tValue;
			y /= _tValue;
			return (*static_cast<_tInher *>(this));
		}

		/**
		 * Performs a dot-product operation.  The dot product of two normalized vectors is the cosine of the angle between
		 *	them.
		 *
		 * \param _v2bOther The vector against which the dot product is to be determined.
		 * \return Returns the dot product between the two vectors.
		 */
		_tType LSE_FCALL							operator * ( const _tInher &_v2bOther ) const {
			return x * _v2bOther.x + y * _v2bOther.y;
		}

		/**
		 * Equality.
		 *
		 * \param _v2bOther The vector to check for equality.
		 * \return Returns true if both vectors are equal.
		 */
		LSBOOL LSE_FCALL							operator == ( const _tInher &_v2bOther ) const {
			return x == _v2bOther.x && y == _v2bOther.y;
		}

		/**
		 * Inequality.
		 *
		 * \param _v2bOther The vector to check for inequality.
		 * \return Returns true if both vectors are not equal.
		 */
		LSBOOL LSE_FCALL							operator != ( const _tInher &_v2bOther ) const {
			return x != _v2bOther.x || y != _v2bOther.y;
		}


		// == Functions.
		/**
		 * Apply the given values to each of the axes on this vector.
		 *
		 * \param _fX The new X-axis value.
		 * \param _fY The new Y-axis value.
		 */
		void LSE_FCALL							Set( _tType _fX, _tType _fY ) {
			x = _fX;
			y = _fY;
		}

		/**
		 * Sets all axes on this vector to 0.
		 */
		void LSE_FCALL							Clear() {
			x = y = _tType( 0 );
		}

		/**
		 * Negates each axis on this vector.
		 */
		void LSE_FCALL							Invert() {
			x = -x;
			y = -y;
		}

		/**
		 * Calculates the length of the vector.
		 *
		 * \return Returns the length of the vector.
		 */
		_tType LSE_FCALL							Len() const {
			register LSREAL fSquared = x * x + y * y;
			return fSquared * CMathLib::InvSqrt( fSquared );
		}

		/**
		 * Calculates the squared length of the vector.
		 *
		 * \return Returns the squared length of the vector.
		 */
		_tType LSE_FCALL							LenSq() const {
			return x * x + y * y;
		}

		/**
		 * Normalizes this vector.  Normalization is the process of adjusting the length of the vector so that it is
		 *	unit length (1 unit in length) while maintaining its direction.
		 * Accuracy/speed depends on the LSM_PERFORMANCE macro.
		 */
		void LSE_FCALL							Normalize() {
			LSREAL fInvLen = CMathLib::InvSqrt( x * x + y * y );
			x *= fInvLen;
			y *= fInvLen;
		}

		/**
		 * Performs a dot-product operation.  The dot product of two normalized vectors is the cosine of the angle between
		 *	them.
		 *
		 * \param _v2bOther The vector against which the dot product is to be determined.
		 * \return Returns the dot product between the two vectors.
		 */
		_tType LSE_FCALL							Dot( const CVector2Base<_tType, _tInher> &_v2bOther ) const {
			return x * _v2bOther.x + y * _v2bOther.y;
		}

		/**
		 * Performs a dot-product operation.  The dot product of two normalized vectors is the cosine of the angle between
		 *	them.
		 *
		 * \param _v2bLeft The left operand.
		 * \param _v2bRight The right operand.
		 * \return Returns the dot product between the two vectors.
		 */
		static _tType LSE_FCALL						DotProduct( const CVector2Base<_tType, _tInher> &_v2bLeft, const CVector2Base<_tType, _tInher> &_v2bRight ) {
			return _v2bLeft.x * _v2bRight.x + _v2bLeft.y * _v2bRight.y;
		}

		/**
		 * Linearly interpolate between two vectors, returning the resulting vector.
		 *
		 * \param _v2bLeft Left operand.
		 * \param _v2bRight Right operand.
		 * \param _fFrac Amount by which to interpolate between the vectors.
		 * \return Returns the vector interpolated between _v2bLeft and _v2bRight by _fFrac.
		 */
		static CVector2Base<_tType, _tInher> LSE_FCALL
													Lerp( const CVector2Base<_tType, _tInher> &_v2bLeft, const CVector2Base<_tType, _tInher> &_v2bRight, _tType _fFrac ) {
			return ((_v2bRight - _v2bLeft) * _fFrac) + _v2bLeft;
		}

		/**
		 * Perform a Hermite spline interpolation between the supplied vectors.
		 *
		 * \param _v2bLeft Left operand.
		 * \param _v2bT0 Left tangent.
		 * \param _v2bRight Right operand.
		 * \param _v2bT1 Right tangent.
		 * \param _fFrac Amount by which to interpolate between the vectors.
		 * \return Returns the vector interpolated between _v2bLeft and _v2bRight by _fFrac.
		 */
		static CVector2Base<_tType, _tInher> LSE_FCALL
													Hermite( const CVector2Base<_tType, _tInher> &_v2bLeft, const CVector2Base<_tType, _tInher> &_v2bT0,
			const CVector2Base<_tType, _tInher> &_v2bRight, const CVector2Base<_tType, _tInher> &_v2bT1,
			_tType _fFrac ) {
			_tType fS2	= _fFrac * _fFrac;
			_tType fS3	= fS2 * _fFrac;
			_tType f2s2	= static_cast<_tType>(2.0) * fS2;
			_tType f2s3	= static_cast<_tType>(2.0) * fS3;
			_tType f3s2	= static_cast<_tType>(3.0) * fS2;

			return (_v2bLeft * (f2s3 - f3s2 + LSM_ONE)) +
				(_v2bRight * (-f2s3 + f3s2)) +
				(_v2bT0 * (fS3 - f2s2 + _fFrac)) +
				(_v2bT1 * (fS3 - fS2));
		}

		/**
		 * Perform a Catmull-Rom spline interpolation between the supplied vectors
		 *
		 * \param _v2bV0 Input point 0.
		 * \param _v2bV1 Input point 1.
		 * \param _v2bV2 Input point 2.
		 * \param _v2bV3 Input point 3.
		 * \param _fFrac Amount by which to interpolate between _v2bV1 and _v2bV2.
		 * \return Returns the vector interpolated between _v2bV1 and _v2bV2 by _fFrac.
		 */
		static CVector2Base<_tType, _tInher> LSE_FCALL
													CatmullRom( const CVector2Base<_tType, _tInher> &_v2bV0, const CVector2Base<_tType, _tInher> &_v2bV1,
			const CVector2Base<_tType, _tInher> &_v2bV2, const CVector2Base<_tType, _tInher> &_v2bV3,
			_tType _fFrac ) {
			_tType fS2	= _fFrac * _fFrac;
			return ((_v2bV1 * static_cast<_tType>(2.0)) +
				(-_v2bV0 + _v2bV2) * _fFrac +
				(_v2bV0 * static_cast<_tType>(2.0) - _v2bV1 * static_cast<_tType>(5.0) + _v2bV2 * static_cast<_tType>(4.0) - _v2bV3) * fS2 +
				(-_v2bV0 + _v2bV1 * static_cast<_tType>(3.0) - _v2bV2 * static_cast<_tType>(3.0) + _v2bV3) * fS2 * _fFrac) * LSM_HALF;
		}


		// == Members.
#pragma pack( push, 1 )
		/**
		 * The X value.
		 */
		_tType										x;

		/**
		 * The Y value.
		 */
		_tType										y;
#pragma pack( pop )
	};

}	// namespace lsm

#endif	// __LSM_VECTOR2BASE_H__

