 ...
 *
 *
 * Description: General converter between any type of floating-point format.
 */

#ifndef __LSM_FLOATX_H__
#define __LSM_FLOATX_H__

#include <cfloat>
#include <cmath>
#include <cstdint>

#define LSM_FLOATX_DBL_EXP_BITS			11
#define LSM_FLOATX_DBL_MAN_BITS			DBL_MANT_DIG
#define LSM_FLOATX_FLT_EXP_BITS			8
#define LSM_FLOATX_FLT_MAN_BITS			FLT_MANT_DIG
#define LSM_FLOAT16_EPSILON				0.0009765625f
...
namespace lsm {
	...
	public :
		...
		static const uint32_t			MaxManBits() { return LSM_FLOATX_DBL_MAN_BITS; }
		...
		static uint64_t					ExpBits( double _dVal ) { return (*reinterpret_cast<uint64_t *>(&_dVal)) & (((1ULL << 11ULL) - 1ULL) << (LSM_FLOATX_DBL_MAN_BITS - 1ULL)); }

		// Gets the mantissa from a double (not shifted).
		static uint64_t					ManBits( double _dVal ) { return (*reinterpret_cast<uint64_t *>(&_dVal)) & (((1ULL << (LSM_FLOATX_DBL_MAN_BITS - 2ULL)) - 1ULL)); }
