...
namespace lsm {
	...
	CFloatX & CFloatX::CreateFromDouble( double _dVal, uint16_t _uiExpBits, uint16_t _uiManBits, bool _bImplicitMantissaBit, bool _bHasSign ) {
		...
		uint64_t uiFinal;
		{
			...
			if ( _uiExpBits == LSM_FLOATX_DBL_EXP_BITS && _uiManBits == LSM_FLOATX_DBL_MAN_BITS && _bImplicitMantissaBit && _bHasSign ) {
				LSM_DOUBLE_INT diVal;
				diVal.dVal = _dVal;
			...
			//const int64_t iShift = RealMantissaBits( LSM_FLOATX_DBL_MAN_BITS, true ) - uiRealMantissa;
			const int64_t iShift = (LSM_FLOATX_DBL_MAN_BITS - 1) - uiRealMantissa;
			...
			dTempVal = (1ULL << (LSM_FLOATX_DBL_MAN_BITS - 1)) / dTemp;
			if ( std::isinf( dTempVal ) ) {
				dTempVal = std::nexttoward( dTempVal, 0.0 );
			}
			const uint64_t uiMulN = (*reinterpret_cast<const uint64_t *>(&dTempVal));				// (1 << LSM_FLOATX_DBL_MAN_BITS) / uiMinN
			dTempVal = dTemp / (1ULL << ((LSM_FLOATX_DBL_MAN_BITS - 1) - iShift));
			//const uint64_t uiMulC = (*reinterpret_cast<const uint64_t *>(&dTempVal));				// uiMinN / (1 << (LSM_FLOATX_DBL_MAN_BITS - iShift))
			...
			/*fTemp.CreateMinNormalized( 11, LSM_FLOATX_DBL_MAN_BITS, true, true );
			dTempVal = fTemp.AsDouble();*/
			dTempVal = DBL_MIN;
...
#if 0
		{
			const uint64_t uiRealMantissa = RealMantissaBits( _uiManBits, _bImplicitMantissaBit );
			const int64_t iShiftSign = 64 - TotalBits( _uiExpBits, _uiManBits, _bImplicitMantissaBit, _bHasSign );
			const int64_t iShiftMan = RealMantissaBits( LSM_FLOATX_DBL_MAN_BITS, true ) - uiRealMantissa;
			const int64_t iShiftExp = 52 + (11 - _uiExpBits);

			LSM_DOUBLE_INT diTmp;
			diTmp.dVal = _dVal;
		...
		*/
		...
		uint64_t uiSExp = (uiAsInt >> (LSM_FLOATX_DBL_MAN_BITS - 1ULL)) & ((1ULL << 11ULL) - 1ULL);
		uint64_t uiSMan = uiAsInt & ((1ULL << (LSM_FLOATX_DBL_MAN_BITS - 1ULL)) - 1ULL);
		...
		uiMantissa = (uiSMan >> (LSM_FLOATX_DBL_MAN_BITS - uiShift));
		if ( uiSExp ) {
			// Rounding.
			uint64_t uiFirstMissingBit = (uiSMan >> (LSM_FLOATX_DBL_MAN_BITS - uiShift - 1ULL)) & 1;
	...
	double CFloatX::AsDouble() const {
		/*if ( uiExpBits == LSM_FLOATX_DBL_EXP_BITS && uiManBits == LSM_FLOATX_DBL_MAN_BITS + 1 && bImplicitManBit && bHasSign ) {
			union LSM_DOUBLE_INT {
				double		dVal;
				int64_t		iVal;
				uint64_t	uiVal;
