...
namespace lsr {
	...
	LSE_CALLCTOR CEntity2d::CEntity2d() :
		m_peParent( NULL ),
		m_ui32EntityType( LSR_ET_ENTITY2D ) {
	}
	LSE_CALLCTOR CEntity2d::~CEntity2d() {
	 ...
	 */
	void LSE_CALL CEntity2d::SetEntityParent( CEntity2d * _peParent ) {
	 ...
	 */
	LSBOOL LSE_CALL CEntity2d::AddEntityChild( CEntity2d * _peChild ) {
		if ( _peChild ) {
	 ...
	 */
	LSBOOL LSE_CALL CEntity2d::RemEntityChild( CEntity2d * _peChild ) {
		LSBOOL bFound = false;
		for ( uint32_t I = m_vEntityChildren.Length(); I--; ) {
	 ...
	 * Resets the entity.
	 */
	void LSE_CALL CEntity2d::ResetEntity() {
		for ( uint32_t I = m_vEntityChildren.Length(); I--; ) {
			m_vEntityChildren[I]->Detach();
		}
	 ...
	 */
	void LSE_CALL CEntity2d::AllocateChildren( uint32_t _ui32Total ) {
	 ...
	 */
	void LSE_CALL CEntity2d::Detach() {
