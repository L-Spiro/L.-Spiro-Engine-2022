...
/*
 * Computes the derived values for a Huffman table.
			 ...
			 c_derived_tbl ** pdtbl)
{
  ...
  /* Allocates a workspace if we haven't already done so. */
  if (*pdtbl == NULL)
    *pdtbl = (c_derived_tbl *)
...
encode_mcu_DC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    /* Computes the DC value after the required point transform by Al.
...
#define MAX_CLEN 32		/* assumed maximum initial code length */
   ...
   * Removesd from this length category two at a time.  The prefix for the pair
  ...
  /* Removes the count for the pseudo-symbol 256 from the largest codelength */
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	/* Allocates and zero the statistics tables */
