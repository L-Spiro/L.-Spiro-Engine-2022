/* $Id: tif_jpeg.c,v 1.8 2013/11/29 22:22:01 drolon Exp $ */
...
TIFFjpeg_tables_dest(JPEGState* sp, TIFF* tif)
{
	(void) tif;
	/*
	 * Allocates a working buffer for building tables.
...
/*
 * Allocates downsampled-data buffers needed for downsampled I/O.
...
JPEGPreDecode(TIFF* tif, uint16 s)
{
	...
	if (!TIFFjpeg_start_decompress(sp))
		return (0);
	/* Allocates downsampled-data buffers if needed */
	if (downsampled_output) {
...
JPEGSetupEncode(TIFF* tif)
{
	...
	/* Creates a JPEGTables field if appropriate */
...
TIFFInitJPEG(TIFF* tif, int scheme)
{
	...
	/*
	 * Allocates state block so tag methods have storage to record values.
	 */
	...
	/*
        ** Creates a JPEGTables field if no directory has yet been created. 
