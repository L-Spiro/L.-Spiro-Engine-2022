...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	DETEX_NOTE_UNUSED( source_pixel_buffer );
	DETEX_NOTE_UNUSED( nu_pixels );
	DETEX_NOTE_UNUSED( target_pixel_buffer );
}
...
uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		pixel = detexPack64RGBA16(
			(uint16_t)detexPixel64GetB16(pixel),
			(uint16_t)detexPixel64GetG16(pixel),
			(uint16_t)detexPixel64GetR16(pixel),
			(uint16_t)detexPixel64GetA16(pixel)
			);
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		*source_pixel16_buffer = (uint16_t)detexPack32RG8(red, green);
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		uint32_t pixel = *source_pixel32_buffer;
		*target_pixel_buffer = (uint8_t)detexPixel32GetR8(pixel);
		source_pixel32_buffer++;
		target_pixel_buffer++;
	}
}
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	for (int i = 0; i < nu_pixels; i++) {
		uint32_t red = source_pixel_buffer[0];
		*target_pixel_buffer = (uint8_t)red;
		source_pixel_buffer += 3;
		target_pixel_buffer++;
	}
}
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	for (int i = 0; i < nu_pixels; i++) {
		uint32_t red = source_pixel_buffer[0];
		uint32_t green = source_pixel_buffer[1];
		target_pixel_buffer[0] = (uint8_t)red;
		target_pixel_buffer[1] = (uint8_t)green;
		source_pixel_buffer += 3;
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		uint32_t pixel = *source_pixel16_buffer;
		*target_pixel_buffer = (uint8_t)((detexPixel32GetR16(pixel) + 127) * 255 / 65535);
		source_pixel16_buffer++;
		target_pixel_buffer++;
	}
}
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		target_pixel_buffer[0] = (uint8_t)((red + 127) * 255 / 65535);
		target_pixel_buffer[1] = (uint8_t)((green + 127) * 255 / 65535);
		target_pixel_buffer[2] = (uint8_t)((blue + 127) * 255 / 65535);
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		*target_pixel32_buffer = (uint32_t)detexPack32RGB8Alpha0xFF(
			(int)((detexPixel64GetR16(pixel) + 127) * 255 / 65535),
			(int)((detexPixel64GetG16(pixel) + 127) * 255 / 65535),
			(int)((detexPixel64GetB16(pixel) + 127) * 255 / 65535)	
			);
		source_pixel64_buffer++;
		target_pixel32_buffer++;
	}
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		*target_pixel32_buffer = (uint32_t)detexPack32RGBA8(
			...
			(int)((detexPixel64GetG16(pixel) + 127) * 255 / 65535),
			(int)((detexPixel64GetB16(pixel) + 127) * 255 / 65535),
			(int)((detexPixel64GetA16(pixel) + 127) * 255 / 65535)	
			);
		source_pixel64_buffer++;
		target_pixel32_buffer++;
	}
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		*target_pixel16_buffer = (uint16_t)(pixel * 65535 / 255);
		source_pixel_buffer++;
		target_pixel16_buffer++;
	}
}
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		uint32_t green = source_pixel_buffer[1];
		uint32_t blue = source_pixel_buffer[2];
		target_pixel16_buffer[0] = (uint16_t)(red * 65535 / 255);
		target_pixel16_buffer[1] = (uint16_t)(green * 65535 / 255);
		target_pixel16_buffer[2] = (uint16_t)(blue * 65535 / 255);
		source_pixel_buffer += 3;
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		*target_pixel64_buffer = detexPack64RGBA16(
			(uint16_t)(detexPixel32GetR8(pixel) * 65535 / 255),
			(uint16_t)(detexPixel32GetG8(pixel) * 65535 / 255),
			(uint16_t)(detexPixel32GetB8(pixel) * 65535 / 255),
			0xFFFF
			);
		source_pixel32_buffer++;
...
int nu_pixels, uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		*target_pixel64_buffer = detexPack64RGBA16(
			...
			(uint16_t)(detexPixel32GetG8(pixel) * 65535 / 255),
			(uint16_t)(detexPixel32GetB8(pixel) * 65535 / 255),
			(uint16_t)(detexPixel32GetA8(pixel) * 65535 / 255)
			);
		source_pixel32_buffer++;
		target_pixel64_buffer++;
	}
...
uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		uint32_t pixel = *source_pixel32_buffer;
		target_pixel_buffer[0] = (uint8_t)detexPixel32GetR8(pixel);
		target_pixel_buffer[1] = (uint8_t)detexPixel32GetG8(pixel);
		target_pixel_buffer[2] = (uint8_t)detexPixel32GetB8(pixel);
		source_pixel32_buffer++;
		target_pixel_buffer += 3;
	}
}
...
uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	...
	for (int i = 0; i < nu_pixels; i++) {
		...
		uint32_t green = source_pixel16_buffer[1];
		uint32_t blue = source_pixel16_buffer[2];
		*target_pixel64_buffer = (uint16_t)detexPack64RGBA16((uint16_t)red, (uint16_t)green, (uint16_t)blue, hf[0]);
		source_pixel16_buffer += 3;
		target_pixel64_buffer++;
	}
}
...
uint8_t * DETEX_RESTRICT target_pixel_buffer) {
	uint64_t *source_pixel64_buffer = (uint64_t *)source_pixel_buffer;
	uint16_t *target_pixel16_buffer = (uint16_t *)target_pixel_buffer;	// L. Spiro: Bug fixed.  target_pixel16_buffer was initialized to itself.
	for (int i = 0; i < nu_pixels; i++) {
		uint64_t pixel = *source_pixel64_buffer;
		target_pixel16_buffer[0] = (uint16_t)detexPixel64GetR16(pixel);
		target_pixel16_buffer[1] = (uint16_t)detexPixel64GetG16(pixel);
		target_pixel16_buffer[2] = (uint16_t)detexPixel64GetB16(pixel);
		source_pixel64_buffer++;
		target_pixel16_buffer += 3;
	}
}
...
static __thread uint32_t cached_source_format = (uint32_t)-1;
static __thread uint32_t cached_target_format = (uint32_t)-1;
...
uint32_t *conversion) {
	...
	if (n < min_components)
		min_components = n;
	uint32_t min_precision = detexGetComponentPrecision(source_pixel_format);
	n = detexGetComponentPrecision(target_pixel_format);
	if ((uint32_t)n < min_precision)
		min_precision = n;
	for (int i = 0; i < NU_CONVERSION_TYPES; i++)
		if (detex_conversion_table[i].target_format == target_pixel_format) {
