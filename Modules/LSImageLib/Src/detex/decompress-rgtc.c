...
int offset, uint8_t * DETEX_RESTRICT pixel_buffer) {
	...
	for (int i = 0; i < 16; i++) {
		...
		if (lum0 > lum1)
			switch (control_code) {
			case 0 : output = (uint8_t)lum0; break;
			case 1 : output = (uint8_t)lum1; break;
			case 2 : output = (uint8_t)detexDivide0To1791By7(6 * lum0 + lum1); break;
			case 3 : output = (uint8_t)detexDivide0To1791By7(5 * lum0 + 2 * lum1); break;
			case 4 : output = (uint8_t)detexDivide0To1791By7(4 * lum0 + 3 * lum1); break;
			case 5 : output = (uint8_t)detexDivide0To1791By7(3 * lum0 + 4 * lum1); break;
			case 6 : output = (uint8_t)detexDivide0To1791By7(2 * lum0 + 5 * lum1); break;
			case 7 : output = (uint8_t)detexDivide0To1791By7(lum0 + 6 * lum1); break;
			}
		else
			switch (control_code) {
			case 0 : output = (uint8_t)lum0; break;
			case 1 : output = (uint8_t)lum1; break;
			case 2 : output = (uint8_t)detexDivide0To1279By5(4 * lum0 + lum1); break;
			case 3 : output = (uint8_t)detexDivide0To1279By5(3 * lum0 + 2 * lum1); break;
			case 4 : output = (uint8_t)detexDivide0To1279By5(2 * lum0 + 3 * lum1); break;
			case 5 : output = (uint8_t)detexDivide0To1279By5(lum0 + 4 * lum1); break;
...
uint32_t flags, uint8_t * DETEX_RESTRICT pixel_buffer) {
	DETEX_NOTE_UNUSED( flags );
	DETEX_NOTE_UNUSED( mode_mask );
