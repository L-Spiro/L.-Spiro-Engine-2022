...
uint8_t * DETEX_RESTRICT endpoint_array) {
	...
	if (GetAlphaComponentPrecision(mode) > 0) {
		...
		uint8_t mask2 = (1 << alpha_precision) - 1;
		for (int j = 0; j < nu_subsets; j++)
			for (int k = 0; k < 2; k++) {	// For each endpoint.
				endpoint_array[j * 8 + k * 4 + 3] = data & mask2;
...
//					alpha_index[i] = block_extract_bits(&block, IB2[mode]);
	...
	for (int i = 0; i < 16; i++) {
		...
		uint32_t output = 0;
		output = detexPack32R8(Interpolate(endpoint_start[0], endpoint_end[0], color_index[i], (uint8_t)color_index_bitcount));
		output |= detexPack32G8(Interpolate(endpoint_start[1], endpoint_end[1], color_index[i], (uint8_t)color_index_bitcount));
		output |= detexPack32B8(Interpolate(endpoint_start[2], endpoint_end[2], color_index[i], (uint8_t)color_index_bitcount));
		output |= detexPack32A8(Interpolate(endpoint_start[3], endpoint_end[3], alpha_index[i], (uint8_t)alpha_index_bitcount));
