...
uint8_t * DETEX_RESTRICT pixel_buffer) {
	...
	for (int i = 0; i < 16; i++) {
		int pixel_index = (int)((qword & (0x0000E00000000000 >> (i * 3))) >> (45 - i * 3));
		...
		buffer[(((i & 3) * 4 + ((i & 12) >> 2)) << shift) + offset] =
			(uint16_t)((value << 5) | (value >> 6));	// Replicate bits to 16-bit.
	}
}
...
uint8_t *pixel_buffer) {
	...
	for (int i = 0; i < 16; i++) {
		...
		buffer[(((i & 3) * 4 + ((i & 12) >> 2)) << shift) + offset] = (uint16_t)bits;
	}
	return true;
}
