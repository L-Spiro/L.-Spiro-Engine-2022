...
//		delta_bits_r = delta_bits_g = delta_bits_b = 10;
	...
	if (signed_flag)
		for (int i = 0; i < 2 * nu_subsets; i++) {
			r[i] = UnquantizeSigned((int16_t)r[i], mode);
			g[i] = UnquantizeSigned((int16_t)g[i], mode);
			b[i] = UnquantizeSigned((int16_t)b[i], mode);
		}
	else
		for (int i = 0; i < 2 * nu_subsets; i++) {
			r[i] = Unquantize((int16_t)r[i], mode);
			g[i] = Unquantize((int16_t)g[i], mode);
			b[i] = Unquantize((int16_t)b[i], mode);
		}
...
//			color_index[i] = block_extract_bits(&block, color_index_bit_count - 1);
			...
			data1 >>= color_index_bit_count - 1ULL;
		}
		else {
//			color_index[i] = block_extract_bits(&block, color_index_bit_count);
	...
	for (int i = 0; i < 16; i++) {
		...
		if (signed_flag) {
			...
			b16 |= s;
			output = detexPack64RGB16((uint16_t)r16, (uint16_t)g16, (uint16_t)b16);
		}
		else {
			output = detexPack64R16(InterpolateFloat(endpoint_start_r, endpoint_end_r, color_index[i],
				(uint8_t)color_index_bit_count) * 31ULL / 64ULL);
...
void detexSetModeBPTC_FLOAT(uint8_t *bitstring, uint32_t mode, uint32_t flags,
	...
	if (mode <= 1) {
		// Set mode 0 or 1.
		bitstring[0] = (uint8_t)((bitstring[0] & 0xFC) | mode);
		return;
	}
