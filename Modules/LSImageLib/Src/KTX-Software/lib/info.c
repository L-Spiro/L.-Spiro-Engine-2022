...
printKTX2Info2(ktxStream* stream, KTX_header2* pHeader)
{
    ...
    if (pHeader->supercompressionGlobalData.byteOffset != 0
        ...
        if (pHeader->supercompressionScheme == KTX_SS_BASIS_LZ) {
            ktx_uint8_t* sgd = malloc((size_t)(pHeader->supercompressionGlobalData.byteLength));
            stream->setpos(stream, (const ktx_off_t)(pHeader->supercompressionGlobalData.byteOffset));
            stream->read(stream, sgd, (ktx_size_t)(pHeader->supercompressionGlobalData.byteLength));
            //
