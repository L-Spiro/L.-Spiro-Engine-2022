...
ktxTexture2_constructCopy(ktxTexture2* This, ktxTexture2* orig)
{
    ...
    if (orig->_private->_sgdByteLength > 0) {
        This->_private->_supercompressionGlobalData
                        = (ktx_uint8_t*)malloc((size_t)orig->_private->_sgdByteLength);
        ...
        memcpy(This->_private->_supercompressionGlobalData,
               ...
               (size_t)orig->_private->_sgdByteLength);
    }
                                        ...
                                        ktxTextureCreateFlags createFlags)
{
    ...
    if (pHeader->supercompressionGlobalData.byteLength > 0) {
        ...
        (void)stream->setpos(stream,
                             (const ktx_off_t)pHeader->supercompressionGlobalData.byteOffset);

        // Read supercompressionGlobalData
        private->_supercompressionGlobalData =
          (ktx_uint8_t*)malloc((size_t)pHeader->supercompressionGlobalData.byteLength);
        ...
        result = stream->read(stream, private->_supercompressionGlobalData,
                              (const ktx_size_t)private->_sgdByteLength);
    ...
    This->dataSize = (ktx_size_t)(private->_levelIndex[0].byteOffset
...
ktxTexture2_calcFaceLodSize(ktxTexture2* This, ktx_uint32_t level)
{
    ...
    if (This->isCubemap && !This->isArray)
        ...
        return (ktx_size_t)This->_private->_levelIndex[level].uncompressedByteLength;
}
                          ...
                          ktx_size_t* pOffset)
{
    ...
    *pOffset = (ktx_size_t)ktxTexture2_levelDataOffset(This, level);
    ...
    switch (This->supercompressionScheme) {
      ...
      case KTX_SS_ZSTD:
      {
            ...
            for (ktx_int32_t level = This->numLevels - 1; level >= 1; level--) {
                ...
                uncompressedLevelSize = (ktx_size_t)levelIndex[level].uncompressedByteLength;
                uncompressedLevelSize = _KTX_PADN(uncompressedLevelAlignment,
                                                  uncompressedLevelSize);
                uncompressedSize += uncompressedLevelSize;
            }
            uncompressedSize += (ktx_size_t)levelIndex[0].uncompressedByteLength;
                                  ...
                                  void* userdata)
{
    ...
    dataSize = (ktx_size_t)levelIndex[0].byteLength;
    dataBuf = malloc(dataSize);
    if (!dataBuf)
        return KTX_OUT_OF_MEMORY;
    if (This->supercompressionScheme == KTX_SS_ZSTD) {
        uncompressedDataSize = (ktx_size_t)levelIndex[0].uncompressedByteLength;
    ...
    for (ktx_int32_t level = This->numLevels - 1; level >= 0; --level)
    {
        ...
        levelSize = (ktx_size_t)levelIndex[level].byteLength;
        ...
        result = stream->setpos(stream,
                                (const ktx_off_t)ktxTexture2_levelFileOffset(This, level));
        if (result != KTX_SUCCESS)
            goto cleanup;
                          ...
                          ktx_uint8_t* pBuffer, ktx_size_t bufSize)
{
    ...
    result = prtctd->_stream.setpos(&prtctd->_stream,
                                    (const ktx_off_t)private->_firstLevelFileOffset);
    if (result != KTX_SUCCESS)
        return result;
    ...
    if (IS_BIG_ENDIAN) {
        ...
        for (ktx_uint32_t level = 0; level < This->numLevels; ++level)
        {
            ...
            levelByteLength = (ktx_size_t)private->_levelIndex[level].byteLength;
            levelOffset = (ktx_size_t)ktxTexture2_levelDataOffset(This, level);
    ...
    for (int32_t level = This->numLevels - 1; level >= 0; level--) {
        size_t levelByteLength =
            ZSTD_decompressDCtx(dctx, pInflatedData + levelOffset,
                              ...
                              (size_t)cindex[level].byteLength);
