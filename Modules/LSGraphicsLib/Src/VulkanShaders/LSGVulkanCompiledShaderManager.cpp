...
namespace lsg {
	 ...
	 */
	LSBOOL CVulkanCompiledShaderManager::m_bSharing = true;

	/**
	 * Map of shaders.
	 */
	CLinearMap<CString, CVulkanCompiledShader *> CVulkanCompiledShaderManager::m_mShaders[LSG_ST_TOTAL];
	 ...
	 */
	void LSE_CALL CVulkanCompiledShaderManager::SetSharing( LSBOOL _bVal ) {
		m_bSharing = _bVal;
	}
	 ...
	 */
	CVulkanCompiledShader * LSE_CALL CVulkanCompiledShaderManager::CreateShader( const CString &_sShaderText, LSG_SHADER_TYPES _stShaderType, CString &_sError ) {
		assert( _stShaderType < LSG_ST_TOTAL );
		// If not sharing, this is simple; just make a new object and return it.
		if ( !m_bSharing ) {
			CVulkanCompiledShader * pogcsRet = LSENEW CVulkanCompiledShader();
			if ( !pogcsRet ) {
	 ...
	 */
	void LSE_CALL CVulkanCompiledShaderManager::DeleteShader( CVulkanCompiledShader * _pogcsShader ) {
		...
		if ( !m_bSharing ) {
			...
			CStd::DebugPrintA( "CVulkanCompiledShaderManager::DeleteShader(): Given shader was not created by CVulkanCompiledShaderManager::CreateShader().\r\n" );
		}
	}
	 ...
	 */
	uint32_t LSE_CALL CVulkanCompiledShaderManager::DeleteUnreferencedShaders() {
