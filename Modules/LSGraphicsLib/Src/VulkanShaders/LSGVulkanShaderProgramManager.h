...
#ifndef __LSG_VULKANSHADERPROGRAMMANAGER_H__
#define __LSG_VULKANSHADERPROGRAMMANAGER_H__
...
namespace lsg {
	...
	/**
	 * Class CVulkanShaderProgramManager
	 * \brief Allows sharing of shader programs in Vulkan.
	 *
	 * Description: In Vulkan, shader programs are made by linking shader parts together.  In L. Spiro Engine, shader
	 *	parts are automatically shared (optionally disablable for multi-context situations), reducing the total possible
	 *	unique combinations between shader parts, meaning that it is very possible for entire shader programs to be
	...
	public :
		...
		typedef struct LSG_ATTRIBUTE_USAGE_LOCS {
			/** The locations. */
			int32_t														iLoc[LSG_MAX_VERTEX_SEMANTIC];
		} * LPLSG_ATTRIBUTE_USAGE_LOCS, * const LPCLSG_ATTRIBUTE_USAGE_LOCS;

		/**
		 * Class CVulkanShaderProgram
		 * \brief A shader program in Vulkan.
		 *
		 * Description: A shader program wrapper for Vulkan.
		 */
		class CVulkanShaderProgram {
		public :
			// == Various constructors.
			LSE_CALLCTOR												CVulkanShaderProgram();
			LSE_CALLCTOR												~CVulkanShaderProgram();
			 ...
			 */
			void LSE_CALL												AddShader( CVulkanCompiledShader * _pogcsShader );
			 ...
			 */
			CVulkanCompiledShader * LSE_CALL							GetShader( LSG_SHADER_TYPES _stShaderType );
			 ...
			 * \return Returns a pointer to the shader set in the given slot.  Returns NULL for empty slots.
			 */
			const CVulkanCompiledShader * LSE_CALL						GetShader( LSG_SHADER_TYPES _stShaderType ) const;
		...
		protected :
			...
			// == Members.
			/** The shaders to link. */
			CVulkanCompiledShader *										m_pogcsShader[LSG_ST_TOTAL];
		...
		private :
			friend class												CVulkanShaderProgramManager;
		};
		 ...
		 */
		static CVulkanShaderProgram * LSE_CALL							CreateShaderProgram( CVulkanCompiledShader * _pogcsVertexShader,
		 ...
		 */
		static void LSE_CALL											DeleteShaderProgram( CVulkanShaderProgram * _pogspProgram );
	...
	protected :
		...
		static CLinearMap<LSG_SHADER_PROGRAM_KEY, CVulkanShaderProgram *>
																		m_mShaders;
		 ...
		 */
		static LSG_SHADER_PROGRAM_KEY LSE_CALL							CreateKey( const CVulkanShaderProgram &_ogspProgram );
	};
	 ...
	 * \return Returns the total attributes in GetAttributeLocs().
	 */
	LSE_INLINE uint32_t LSE_CALL CVulkanShaderProgramManager::CVulkanShaderProgram::TotalAttributes() const {
...
#endif	// __LSG_VULKANSHADERPROGRAMMANAGER_H__
