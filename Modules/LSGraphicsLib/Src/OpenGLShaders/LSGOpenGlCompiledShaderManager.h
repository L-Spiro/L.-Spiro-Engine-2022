...
#ifndef __LSG_OPENGLCOMPILEDSHADERMANAGER_H__
#define __LSG_OPENGLCOMPILEDSHADERMANAGER_H__
...
namespace lsg {
	 ...
	 * \brief Manages the sharing of similar OpenGL shaders parts (a single vertex, pixel, or geometry shader).
	 *
	 * Description: Manages the sharing of similar OpenGL shaders parts (a single vertex, pixel, or geometry shader).
	 *	Since OpenGL does not give us back the compiled code (sigh) we can only share based on the input text.  Sadly
	 *	this reduces the efficiency of the sharing, but it is not so bad thanks to the preprocessing the core engine
	...
	public :
		 ...
		 */
		static COpenGlCompiledShader * LSE_CALL					CreateShader( const CString &_sShaderText, LSG_SHADER_TYPES _stShaderType, CString &_sError );
		 ...
		 */
		static void LSE_CALL									DeleteShader( COpenGlCompiledShader * _pogcsShader );
	...
	protected :
		...
		/** Map of shaders. */
		static CLinearMap<CString, COpenGlCompiledShader *>		m_mShaders[LSG_ST_TOTAL];
	};
...
#endif	// __LSG_OPENGLCOMPILEDSHADERMANAGER_H__
