...
#include "LSGOpenGlShaderTranslator.h"
...
namespace lsg {
	...
	LSE_CALLCTOR COpenGlShaderTranslator::COpenGlShaderTranslator() :
		m_bIsMainNow( false ) {
	}
	LSE_CALLCTOR COpenGlShaderTranslator::~COpenGlShaderTranslator() {
	}
	 ...
	 */
	LSBOOL LSE_CALL COpenGlShaderTranslator::PostShaderString( CString &_sShader, CString &_sError ) {
		static const char * pcHeader = "#version 330\n";
		CString sHeader;
		...
		if ( m_pslpContainer->GetShaderType() == LSG_ST_VERTEX ) {
			...
			sHeader += CString( "out vec4 lsg_vColor[" ) + CString::CreateFromUI64( CGfx::GetMetrics().ui32MaxRenderTargets ) + CString( "];\n" );
		}

		// Insert shadowmapping intrinsics.
		sHeader += "float shadow2dDepth(sampler2D s,vec2 v){return texture(s,v).z;}\n"
		...
		for ( uint32_t I = m_alAttributes.Length(); I--; ) {
			...
			if ( m_alAttributes[I].bInput ) {
				if ( !sHeader.Append( "in " ) ) {
					_sError = LSG_STB_OOM;
					return false;
				}
			}
			else if ( !sHeader.Append( "out " ) ) {
				_sError = LSG_STB_OOM;
				return false;
			}
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleFunctionDeclaration( LSG_NODE_CALLBACK &_ncData ) {
		m_bIsMainNow = false;
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleParamaterDeclarator( LSG_NODE_CALLBACK &/*_ncData*/ ) {
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleSingleDeclaration( LSG_NODE_CALLBACK &_ncData ) {
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleFullySpecifiedType( LSG_NODE_CALLBACK &_ncData ) {
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleIntrinsic( LSG_NODE_CALLBACK &_ncData ) {
		...
		static const struct LSG_INTRINSIC {
			...
			{ "clamp",				CShaderParser::token::LSG_SLT_SATURATE,				1 },
			...
			{ "texture",			CShaderParser::token::LSG_SLT_TEX1D,				3 },
			{ "texture",			CShaderParser::token::LSG_SLT_TEX2D,				3 },
			{ "texture",			CShaderParser::token::LSG_SLT_TEX3D,				3 },
			{ "texture",			CShaderParser::token::LSG_SLT_TEXCUBE,				3 },
		...
		for ( uint32_t I = LSE_ELEMENTS( sFuncs ); I--; ) {
			if ( sFuncs[I].ui32Id == _ncData.pndNode->nuNodeData.ui32NodeIndex ) {
				...
				if ( _ncData.pndNode->nuNodeData.ui32NodeIndex == CShaderParser::token::LSG_SLT_SATURATE ) {
					// Special case in which saturate( X ) becomes clamp( X, 0, 1 ).
					if ( !PushNode( (*_ncData.pnsNodeStack), ui32Args[0], _ncData.ui32Index, 0, ",0.0,1.0)", szPrefix ) ) {
						_ncData.sError = LSG_STB_OOM;
						return LSG_PD_ERROR;
					}
				}
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleCast( LSG_NODE_CALLBACK &_ncData ) {
	 ...
	 *	of the node or not.
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::HandleIdentifier( LSG_NODE_CALLBACK &_ncData ) {
	 ...
	 */
	LSBOOL LSE_CALL COpenGlShaderTranslator::DecodeTypeQualifier( uint64_t _ui64Qualifier, CString &_sString ) {
	 ...
	 * \return Returns true if there was enough memory to perform the operation.
	 */
	LSBOOL LSE_CALL COpenGlShaderTranslator::DecodeType( uint64_t _ui64Type, CString &_sString ) {
		static struct LSG_TYPES {
			...
			{ CShaderParser::token::LSG_SLT_FLOAT,				"float ",		"vec%u ",	"mat%ux%u " },
	 ...
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::VertexInTexCoord( LSG_NODE_CALLBACK &/*_ncData*/,
	 ...
	 * \return Returns a LSG_PROCESSING_DIRECTIVE enumerated value indicating success or failure. 
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::VertexOutTexCoord( LSG_NODE_CALLBACK &_ncData,
		uint32_t _ui32ParmNodeIndex,
		uint32_t _ui32Index, CString &_sRet, CString &_sError ) {
	 ...
	 * \return Returns a LSG_PROCESSING_DIRECTIVE enumerated value indicating success or failure. 
	 */
	COpenGlShaderTranslator::LSG_PROCESSING_DIRECTIVE LSE_CALL COpenGlShaderTranslator::PixelInTexCoord( LSG_NODE_CALLBACK &_ncData,
		uint32_t _ui32ParmNodeIndex,
		uint32_t _ui32Index, CString &_sRet, CString &_sError ) {
