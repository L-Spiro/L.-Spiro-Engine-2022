...
#ifndef __LSG_OPENGLSHADERMANAGER_H__
#define __LSG_OPENGLSHADERMANAGER_H__
...
namespace lsg {
	 ...
	 * \brief Manages the interface between shader objects and the OpenGL API.
	 *
	 * Description: Manages the interface between shader objects and the OpenGL API.  Its main function is to apply
	 *	and unapply shaders, but it also applies engine-specific functionality to shaders as well, where they are
	 *	not supported by GLSL.  For example, automatic application of light and fog values, etc.
	...
	public :
		...
		LSE_CALLCTOR										COpenGlShaderManager();
		LSE_CALLCTOR										~COpenGlShaderManager();
...
#endif	// __LSG_OPENGLSHADERMANAGER_H__
