...
#ifndef __LSG_OPENGLSHADER_H__
#define __LSG_OPENGLSHADER_H__
...
#include "LSGOpenGlCompiledShaderManager.h"
#include "LSGOpenGlShaderProgramManager.h"
#include "LSGOpenGlShaderTranslator.h"

namespace lsg {

	class COpenGlShaderManager;

	/**
	 * Class COpenGlShader
	 * \brief Open OpenGL shader.
	 *
	 * Description: Open OpenGL shader.  Specifically for OpenGL, after shaders are translated, they are linked
	 *	into an OpenGL program object.  A shader program in OpenGL is a combination of a vertex shader, pixel
	 *	shader, and geometry shader.  Each of these are called shader parts in this context, and shader parts
	 ...
	 *	reduces significantly the number of shader swaps that need to be made during rendering.
	 */
	class COpenGlShader : public CShaderBase {
	public :
		// == Various constructors.
		LSE_CALLCTOR							COpenGlShader();
		virtual LSE_CALLCTOR					~COpenGlShader();
		 ...
		 */
		const COpenGlShaderProgramManager::LSG_ATTRIBUTE_USAGE_LOCS * LSE_CALL
	...
	protected :
		 ...
		 */
		COpenGlCompiledShader *					m_pogcsShaders[LSG_ST_TOTAL];

		/**
		 * The shader program with all of the shader parts linked.
		 */
		COpenGlShaderProgramManager::COpenGlShaderProgram *
												m_pospProgram;

		/**
		 * The shader manager to which this shader is applied, if any.
		 */
		COpenGlShaderManager *					m_pogsmShaderMan;
	...
	private :
		...
		friend class							COpenGlShaderManager;
	};
	 ...
	 * \return Returns the total attribute locations for this shader.
	 */
	LSE_INLINE uint32_t LSE_CALL COpenGlShader::TotalAttributes() const {
