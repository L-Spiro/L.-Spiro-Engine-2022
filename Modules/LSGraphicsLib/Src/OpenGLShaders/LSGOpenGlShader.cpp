 ...
 *
 *
 * Description: Open OpenGL shader.  Specifically for OpenGL, after shaders are translated, they are linked
 *	into an OpenGL program object.  A shader program in OpenGL is a combination of a vertex shader, pixel
 *	shader, and geometry shader.  Each of these are called shader parts in this context, and shader parts
 *	can be shared.  For this reason, it may be common for multiple OpenGL shader programs to be comprised
...
#include "LSGOpenGlShader.h"

#if defined( LSG_OGL )

#include "../Gfx/LSGGfx.h"
#include "LSGOpenGlShaderManager.h"

namespace lsg {

	// == Various constructors.
	LSE_CALLCTOR COpenGlShader::COpenGlShader() :
	 ...
	 * Reset everything.
	 */
	void LSE_CALL COpenGlShader::Reset() {
		if ( m_pogsmShaderMan ) {
			// Unattach this shader from the manager.
			COpenGlShaderManager * pogsmMan = m_pogsmShaderMan;
			m_pogsmShaderMan = NULL;
			pogsmMan->SetShader( NULL );
		}
		// Destroy the shader program.
		COpenGlShaderProgramManager::DeleteShaderProgram( m_pospProgram );
		m_pospProgram = NULL;

		// Dereference shader parts.
		for ( uint32_t I = LSG_ST_TOTAL; I--; ) {
			COpenGlCompiledShaderManager::DeleteShader( m_pogcsShaders[I] );
			m_pogcsShaders[I] = NULL;
		}
		Parent::Reset();
	}
	 ...
	 */
	uint64_t LSE_CALL COpenGlShader::GetId() const {
	 ...
	 */
	const COpenGlShaderProgramManager::LSG_ATTRIBUTE_USAGE_LOCS * LSE_CALL COpenGlShader::GetAttributeLocs() const {
	 ...
	 */
	LSBOOL LSE_CALL COpenGlShader::Activate( void * _pvSysValue ) {
#ifdef _DEBUG
		CCriticalSection::CLocker lGetError( COpenGl::m_csCrit );
		glWarnError( "Uncaught" );	// Clear error flag.
#endif	// #ifdef _DEBUG

		COpenGl::glUseProgram( m_pospProgram->GetId() );

		glWarnError( "Error activating OpenGL shader" );
		m_pogsmShaderMan = static_cast<COpenGlShaderManager *>(_pvSysValue);
	 ...
	 */
	void LSE_CALL COpenGlShader::Deactivate() {
		m_pogsmShaderMan = NULL;
	}
	 ...
	 */
	COpenGlShader::LSG_HANDLE LSE_CALL COpenGlShader::GetConstantByName( const char * _pcName ) {
	 ...
	 */
	LSBOOL LSE_CALL COpenGlShader::SetFloat( LSG_HANDLE _hHandle, float _fValue ) {
	 ...
	 */
	LSBOOL LSE_CALL COpenGlShader::SetFloatArray( LSG_HANDLE _hHandle, const float * _pfValues, uint32_t _ui32Total ) {
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetInt( LSG_HANDLE _hHandle, int32_t _iValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the values were set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetIntArray( LSG_HANDLE _hHandle, const int32_t * _pi32Values, uint32_t _ui32Total ) {
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetBool( LSG_HANDLE _hHandle, LSBOOL _bValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the values were set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetBoolArray( LSG_HANDLE _hHandle, const LSBOOL * _pbValues, uint32_t _ui32Total ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetVector2( LSG_HANDLE _hHandle, const CVector2 &_vValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the values were set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetVector2Array( LSG_HANDLE _hHandle, const CVector2 * _pvValues, uint32_t _ui32Total ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetVector3( LSG_HANDLE _hHandle, const CVector3 &_vValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the values were set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetVector3Array( LSG_HANDLE _hHandle, const CVector3 * _pvValues, uint32_t _ui32Total ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetVector4( LSG_HANDLE _hHandle, const CVector4 &_vValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the values were set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetVector4Array( LSG_HANDLE _hHandle, const CVector4 * _pvValues, uint32_t _ui32Total ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetMatrix2x2( LSG_HANDLE _hHandle, const CMatrix2x2 &_mValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetMatrix3x3( LSG_HANDLE _hHandle, const CMatrix3x3 &_mValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the value was set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetMatrix4x4( LSG_HANDLE _hHandle, const CMatrix4x4 &_mValue ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	 ...
	 * \return Returns true if the values were set in the shader.
	 */
	LSBOOL LSE_CALL COpenGlShader::SetMatrix4x4Array( LSG_HANDLE _hHandle, const CMatrix4x4 * _pmValues, uint32_t _ui32Total ) {
		if ( _hHandle == LSE_MAXU32 || !m_pospProgram ) { return false; }
	...
	/**
	 * Creates a new translator of type COpenGlShaderTranslator.
	 */
	void LSE_CALL COpenGlShader::CreateTranslator() {
		DestroyTranslator();
		COpenGlShaderTranslator * pogstNew = LSENEW COpenGlShaderTranslator();
		m_pstpbTranslator = pogstNew;
	}

	/**
	 * Destroys the COpenGlShaderTranslator translator.
	 */
	void LSE_CALL COpenGlShader::DestroyTranslator() {
		if ( m_pstpbTranslator ) {
			COpenGlShaderTranslator * pogstDelete = static_cast<COpenGlShaderTranslator *>(m_pstpbTranslator);
	...
	/**
	 * Post compile.  We use this to create OpenGL compiled shader parts and link them to
	 *	create a shader program.
	 *
	 * \param _sError The returned error string if there is an error.
	 * \return Returns an error code or LSG_SE_SUCCESS.
	 */
	LSG_SHADER_ERRORS LSE_CALL COpenGlShader::PostCompile( CString &_sError ) {
		if ( m_sVertexShader.Length() ) {
			m_pogcsShaders[LSG_ST_VERTEX] = COpenGlCompiledShaderManager::CreateShader( m_sFinalVertex, LSG_ST_VERTEX, _sError );
		...
		if ( m_sPixelShader.Length() ) {
			m_pogcsShaders[LSG_ST_PIXEL] = COpenGlCompiledShaderManager::CreateShader( m_sFinalPixel, LSG_ST_PIXEL, _sError );
		...
		m_pospProgram = COpenGlShaderProgramManager::CreateShaderProgram( m_pogcsShaders[LSG_ST_VERTEX],
