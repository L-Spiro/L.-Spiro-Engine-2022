 ...
 *
 *
 * Description: Manages the sharing of similar OpenGL shaders parts (a single vertex, pixel, or geometry shader).
 *	Since OpenGL does not give us back the compiled code (sigh) we can only share based on the input text.  Sadly
 ...
 *	tools may have multiple OpenGL contexts and sharing will cause errors between them.
 */
...
namespace lsg {
	 ...
	 */
	LSBOOL COpenGlCompiledShaderManager::m_bSharing = true;

	/**
	 * Map of shaders.
	 */
	CLinearMap<CString, COpenGlCompiledShader *> COpenGlCompiledShaderManager::m_mShaders[LSG_ST_TOTAL];
	 ...
	 */
	void LSE_CALL COpenGlCompiledShaderManager::SetSharing( LSBOOL _bVal ) {
		m_bSharing = _bVal;
	}
	 ...
	 */
	COpenGlCompiledShader * LSE_CALL COpenGlCompiledShaderManager::CreateShader( const CString &_sShaderText, LSG_SHADER_TYPES _stShaderType, CString &_sError ) {
		assert( _stShaderType < LSG_ST_TOTAL );
		// If not sharing, this is simple; just make a new object and return it.
		if ( !m_bSharing ) {
			COpenGlCompiledShader * pogcsRet = LSENEW COpenGlCompiledShader();
			if ( !pogcsRet ) {
	 ...
	 */
	void LSE_CALL COpenGlCompiledShaderManager::DeleteShader( COpenGlCompiledShader * _pogcsShader ) {
		...
		if ( !m_bSharing ) {
			...
			CStd::DebugPrintA( "COpenGlCompiledShaderManager::DeleteShader(): Given shader was not created by COpenGlCompiledShaderManager::CreateShader().\r\n" );
		}
	}
	 ...
	 */
	uint32_t LSE_CALL COpenGlCompiledShaderManager::DeleteUnreferencedShaders() {
