...
#ifndef __LSG_DIRECTX9SHADER_H__
#define __LSG_DIRECTX9SHADER_H__
...
namespace lsg {
	...
	/**
	 * Class CDirectX9Shader
	 * \brief A DirectX 9 shader, composed of a single vertex shader, pixel shader, and optional geometry shader.
	 *
	 * Description: A DirectX 9 shader.  Shaders in DirectX 9 are normally considered to be specifically a vertex shader
	 *	or a pixel shader (etc.), but here a single vertex, pixel, and geometry shader are treated as one, matching
	 *	shader behavior in OpenGL.  Vertex, pixel, and geometry shaders are called shader parts here, with the shader being
	 ...
	 *	sharing.  Sharing can be disabled for debugging purposes.
	 */
	class CDirectX9Shader : public CShaderBase {
	public :
		// == Various constructors.
		LSE_CALLCTOR										CDirectX9Shader();
		virtual LSE_CALLCTOR								~CDirectX9Shader();
	...
	protected :
		 ...
		 */
		typedef struct LSG_DX9_SHADER_HANDLE {
		...
		} * LPLSG_DX9_SHADER_HANDLE, * const LPCLSG_DX9_SHADER_HANDLE;
		 ...
		 */
		CDirectX9VertexShader *								m_pdx9vsVertexShader;

		/**
		 * The pixel shader, if any.  There must be a vertex shader if there is a pixel shader.
		 */
		CDirectX9PixelShader *								m_pdx9psPixelShader;

		/**
		 * Array of handles.  The handles we give out are indices into this array.
		 */
		CVectorPoD<LSG_DX9_SHADER_HANDLE>					m_vHandles;

		/**
		 * The shader manager.
		 */
		CDirectX9ShaderManager *							m_pdx9smShaderManager;
...
#endif	// __LSG_DIRECTX9SHADER_H__
