...
namespace lsg {
	 ...
	 */
	LSBOOL CDirectX9CompiledShaderManager::m_bSharing = true;

	/**
	 * Map of shaders.
	 */
	CLinearMap<CString, CCompiledShaderBase *> CDirectX9CompiledShaderManager::m_mShaders[LSG_ST_TOTAL];
	 ...
	 */
	void LSE_CALL CDirectX9CompiledShaderManager::SetSharing( LSBOOL _bVal ) {
		m_bSharing = _bVal;
	}
	 ...
	 */
	CCompiledShaderBase * LSE_CALL CDirectX9CompiledShaderManager::CreateShader( const CString &_sShaderText, LSG_SHADER_TYPES _stShaderType,
		...
		if ( !m_bSharing ) {
			...
			switch ( _stShaderType ) {
				case LSG_ST_PIXEL : {
					pogcsRet = LSENEW CDirectX9PixelShader();
					break;
				}
				case LSG_ST_VERTEX : {
					pogcsRet = LSENEW CDirectX9VertexShader();
					break;
				}
			}
			if ( !pogcsRet ) {
			...
			if ( !pogcsRet->Compile( _sShaderText, _stShaderType, _sError ) ) {
				...
				if ( !static_cast<CDirectX9CompiledShader *>(pogcsRet)->SetUniforms( _pulUniforms[_stShaderType] ) ) {
					LSEDELETE pogcsRet;
					pogcsRet = NULL;
				}
				else {
	 ...
	 */
	void LSE_CALL CDirectX9CompiledShaderManager::DeleteShader( CCompiledShaderBase * _pogcsShader ) {
		...
		if ( !m_bSharing ) {
			switch ( _pogcsShader->GetType() ) {
				case LSG_ST_PIXEL : {
					static_cast<CDirectX9PixelShader *>(_pogcsShader)->DecRef();
					break;
				}
				case LSG_ST_VERTEX : {
					static_cast<CDirectX9VertexShader *>(_pogcsShader)->DecRef();
					break;
				}
			}
			...
			CStd::DebugPrintA( "CDirectX9CompiledShaderManager::DeleteShader(): Given shader was not created by CDirectX9CompiledShaderManager::CreateShader().\r\n" );
		}
	}
	 ...
	 */
	uint32_t LSE_CALL CDirectX9CompiledShaderManager::DeleteUnreferencedShaders() {
