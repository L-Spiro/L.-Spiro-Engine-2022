...
#include "LSGDirectX11FramebufferBase.h"
...
namespace lsg {
	...
	LSE_CALLCTOR CDirectX11FramebufferBase::CDirectX11FramebufferBase() {
	}
	LSE_CALLCTOR CDirectX11FramebufferBase::~CDirectX11FramebufferBase() {
	}
	 ...
	 */
	LSBOOL LSE_CALL CDirectX11FramebufferBase::Activate() {
		for ( uint32_t I = LSE_ELEMENTS( m_aColors ); I--; ) {
			if ( !m_aColors[I].prtiRenderTarget ) {
				CDirectX11::m_rsCurRenderState.prtRenderTargets[I] = NULL;
			}
			else {
		...
		if ( !m_aDepthStencil.prtiRenderTarget ) {
			CDirectX11::m_rsCurRenderState.pdsDepthStencil = NULL;
		}
		else {
