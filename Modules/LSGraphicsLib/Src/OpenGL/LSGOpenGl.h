...
#ifndef __LSG_OPENGL_H__
#define __LSG_OPENGL_H__
...
namespace lsg {
	...
	enum LSG_BLEND_OP {
		...
		LSG_BO_MIN									= GL_MIN,										/**< Find the minimum of source 1 and source 2. */
		LSG_BO_MAX									= GL_MAX,										/**< Find the maximum of source 1 and source 2. */
	...
	/** Fill modes. */
	enum LSG_FILL_MODE {
		LSG_FM_SOLID								= GL_FILL,										/**< Solid filling mode. */
		LSG_FM_WIRE									= GL_LINE,										/**< Wire-frame filling mode. */
	};
	...
	enum LSG_TEXTURE_ADDRESS_MODE {
		...
		LSG_TAM_BORDER								= GL_CLAMP_TO_BORDER,							/**< Texture coordinates outside the range [0.0, 1.0] are set to the border color. */
...
#define LSG_DEFAULT_STENCIL_WRITE_MASK				(1)												/**< Default stencil write mask. */
	...
	class											CFramebuffer;
	typedef class COpenGlShaderManager				CShaderManager;
	class											CShader;
	class											CVertexBuffer;
...
#include "../Gfx/LSGGfxState.h"
	...
	/**
	 * Class COpenGl
	 * \brief Interface with the OpenGL * API.
	 *
	 * Description: Interface with the OpenGL * API.
	 */
	class COpenGl : public CGfxBase {
		friend class								COpenGlFramebuffer;
		friend class								COpenGlFramebufferBase;
		friend class								COpenGlIndexBuffer;
		friend class								COpenGlTexture2d;
		friend class								COpenGlVertexBuffer;
	public :
		// == Types.
		...
		/** A helper for temporarily binding a 2D array texture (for creating it, modifying it, etc.) */
		class CTempTexture2dArrayBinder {
		public :
			// == Various contructors.
			LSE_CALLCTOR							CTempTexture2dArrayBinder( GLuint _uiId );
			LSE_CALLCTOR							~CTempTexture2dArrayBinder();
		 ...
		 */
		static LSBOOL LSE_CALL						InitOpenGl( void * _pvInit );

		/**
		 * Secondary initialization of OpenGL, after a device has been created.
		 */
		static void LSE_CALL						PostInitOpenGl();

		/**
		 * Destroy OpenGL one time at engine shutdown.
		 */
		static void LSE_CALL						DestroyOpenGl();
...
#ifdef _DEBUG
#define glWarnError( _pcHeader )					COpenGl::WarnErrorEx( _pcHeader, __FILE__, __LINE__ )
#define glAssertError( _pcHeader )					assert( !COpenGl::WarnErrorEx( _pcHeader, __FILE__, __LINE__ ) )
#else
#define glWarnError( _pcHeader )					false
#define glAssertError( _pcHeader )
#endif	// #ifdef _DEBUG
#define glWarnErrorAlways( _pcHeader )				COpenGl::WarnErrorEx( _pcHeader, __FILE__, __LINE__ )
...
#include "../Gfx/LSGCreateStatesDecl.h"
		...
		/**
		 * Binds a texture 2D array to a slot.
		 *
		 * \param _uiId The texture to bind.
		 * \param _ui32Slot The slot into which to bind the texture.
		 */
		static LSE_INLINE void LSE_CALL				BindTexture2dArray( GLuint _uiId, uint32_t _ui32Slot );
		 ...
		 * \return Returns the OpenGL ID of the texture bound to _ui32Slot.
		 */
		static LSE_INLINE GLuint LSE_CALL			TextureBinding2dArray( uint32_t _ui32Slot );
		 ...
		 * Binds a framebuffer.
		 *
		 * \param _uiFbo The ID of the framebuffer to bind.
		 */
		static LSE_INLINE void LSE_CALL				BindFramebuffer( GLuint _uiFbo );
...
#include "../Gfx/LSGDescFuncDecl.h"
	...
	protected :
		// == Types.
		/** Prototype of the function used to get function addresses. */
		typedef void * (APIENTRY *					wglGetProcAddressLSG)( const char * _pcName );
		...
		typedef struct LSG_OPENGL_RENDER_STATE : public LSG_RENDER_STATE {
			...
			GLuint									uiTexture2dArray[LSG_MAX_TEXTURE_UNITS];		/**< List of active 2D array OpenGL textures. */
...
#include "../Gfx/LSGStateRefDecl.h"
		...
		static wglGetProcAddressLSG					GetProcAddressLSG;
...
#include "../Gfx/LSGCreateStatesMembersDecl.h"
		...
		/**
		 * Updates vertex attributes for OpneGL.
		 */
	 ...
	 * \return Returns a pointer to the shader manager used by this class.
	 */
	LSE_INLINE CShaderManager * LSE_FCALL COpenGl::GetShaderManager() {
	 ...
	 * \return Returns true if the rendering device is ready.
	 */
	LSE_INLINE LSBOOL LSE_FCALL COpenGl::BeginRender() {
		return true;
	}
	 ...
	 * End rendering.  Must be called once per frame only, after all rendering is done.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::EndRender() {
	}

	/**
	 * Present the backbuffer to the screen.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::Present() {
#ifndef LSA_TOOLS
#ifdef LSE_WINDOWS
		::SwapBuffers( m_hDc );
#endif	// #ifdef LSE_WINDOWS
	 ...
	 * \return Returns the input matrix so that this function can be used as a parameter to another function.
	 */
	LSE_INLINE CMatrix4x4 & LSE_CALL COpenGl::PerspectiveFov( CMatrix4x4 &_mIn, LSREAL _fFov, LSREAL _fAspect, LSREAL _fNear, LSREAL _fFar ) {
		// Right-handed.
	 ...
	 * \return Returns the input matrix so that this function can be used as a parameter to another function.
	 */
	LSE_INLINE CMatrix4x4 & LSE_CALL COpenGl::OrthoOffCenter( CMatrix4x4 &_mIn,
		LSREAL _fLeft, LSREAL _fRight,
		LSREAL _fBottom, LSREAL _fTop,
		LSREAL _fNear, LSREAL _fFar ) {
		// Right-handed.
	 ...
	 * \return Returns the reference to _fFrustum.
	 */
	LSE_INLINE CFrustum & LSE_CALL COpenGl::ExtractFrustumPlanes( CMatrix4x4 &_mMatrix, CFrustum &_fFrustum, LSBOOL _bNormalize ) {
		// Left clipping plane.
	 ...
	 *	resources, etc.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetDefaultObjectRenderStates() {
	 ...
	 * Sets all default render states.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetAllDefaultRenderStates() {
		// General states.
	 ...
	 * Activates the default backbuffer and depth/stencil buffers.
	 */
	LSE_INLINE void LSE_CALL COpenGl::ActivateBackBuffers() {
		BindFramebuffer( 0 );
...
#pragma warning( disable : 4127 )
	 ...
	 * \param _ui32Mask The buffer(s) to clear.  Must be one or more LSG_BUFFERS enumerated values.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::Clear( uint32_t _ui32Mask ) {
		ApplyFramebuffer();
		glWarnError( "Uncaught" );
#ifdef LSA_TOOLS
#define LSG_COMP( MEM )		true
#else
...
#define LSG_COPY( MEM )		m_rsLastRenderState.MEM = m_rsCurRenderState.MEM
		...
		if ( _ui32Mask & LSG_B_DEPTH_BUFFER ) {
			if ( LSG_COMP( fClearDepth ) ) {
				::glClearDepth( m_rsCurRenderState.fClearDepth );
...
#pragma warning( pop )
	 ...
	 * Clear all active buffers.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::Clear() {
		Clear( LSG_B_ALL );
	}
	 ...
	 * \param _fA Alpha value.
	 */
	LSE_INLINE void LSE_CALL COpenGl::SetClearColor( LSREAL _fR, LSREAL _fG, LSREAL _fB, LSREAL _fA ) {
	 ...
	 * \param _fValue The new depth clear value.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetClearDepth( LSREAL _fValue ) {
	 ...
	 * \param _ui8Val The new stencil clear value.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetClearStencil( uint8_t _ui8Val ) {
	 ...
	 * \param _ui32Target Index of the viewport to set.
	 */
	LSE_INLINE void LSE_CALL COpenGl::SetViewport( LSREAL _fX, LSREAL _fY,
	 ...
	 * \param _ui32Target Index of the depth range to set.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetDepthRange( LSREAL _fMin, LSREAL _fMax, uint32_t _ui32Target ) {
		assert( _ui32Target < LSG_MAX_VIEWPORTS_SCISSORS );
	 ...
	 * \param _ui32Total The total number of viewports rectangles set.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetTotalViewports( uint32_t _ui32Total ) {
	 ...
	 * \return Returns LSG_CM_FRONT for LSG_CM_BACK, LSG_CM_BACK for LSG_CM_FRONT, and LSG_CM_NONE for LSG_CM_NONE.
	 */
	LSE_INLINE LSG_CULL_MODE LSE_FCALL COpenGl::ReverseCull( LSG_CULL_MODE _cmMode ) {
		switch ( _cmMode ) {
	 ...
	 * \return Returns a reference to the currently set number of vertex buffers.
	 */
	LSE_INLINE uint32_t & LSE_CALL COpenGl::TotalActiveVertexBuffers() {
	 ...
	 * \return Returns the array of current vertex buffers for read and write.
	 */
	LSE_INLINE LSG_VERTEX_BUFFER * LSE_CALL COpenGl::VertexBuffers() {
	 ...
	 * \param _ui32Slot The slot to activate.
	 */
	LSE_INLINE void LSE_CALL COpenGl::ActivateTexture( uint32_t _ui32Slot ) {
		...
		if ( m_rsCurRenderState.eActiveTexture != static_cast<GLenum>(_ui32Slot) ) {
			...
			COpenGl::glActiveTexture( m_rsCurRenderState.eActiveTexture + GL_TEXTURE0 );

			glWarnError( "COpenGl::ActivateTexture() failed" );
#ifndef LSA_TOOLS
		}
#endif	// #ifndef LSA_TOOLS
	}
	 ...
	 * \return Returns the generated texture ID.
	 */
	LSE_INLINE GLuint LSE_CALL COpenGl::GenTexture() {
		GLuint uiId;
	 ...
	 * \param _uiId The OpenGL texture ID to delete.
	 */
	LSE_INLINE void LSE_CALL COpenGl::DeleteTexture( GLuint &_uiId ) {
		if ( _uiId ) {
	 ...
	 * \param _ui32Slot The slot into which to bind the texture.
	 */
	LSE_INLINE void LSE_CALL COpenGl::BindTexture2d( GLuint _uiId, uint32_t _ui32Slot ) {
		assert( _ui32Slot < m_mMetrics.ui32MaxTexSlot );
#ifndef LSA_TOOLS
	 ...
	 * \return Returns the OpenGL ID of the texture bound to _ui32Slot.
	 */
	LSE_INLINE GLuint LSE_CALL COpenGl::TextureBinding2d( uint32_t _ui32Slot ) {
		assert( _ui32Slot < m_mMetrics.ui32MaxTexSlot );
	 ...
	 * \param _ui32Slot The slot into which to bind the texture.
	 */
	LSE_INLINE void LSE_CALL COpenGl::BindTexture2dArray( GLuint _uiId, uint32_t _ui32Slot ) {
		assert( _ui32Slot < m_mMetrics.ui32MaxTexSlot );
#ifndef LSA_TOOLS
		if ( m_rsCurRenderState.uiTexture2dArray[_ui32Slot] != _uiId ) {
#endif	// #ifndef LSA_TOOLS
			CCriticalSection::CLocker lLock( m_csBindTextureCrit );
			ActivateTexture( _ui32Slot );
			::glBindTexture( GL_TEXTURE_2D_ARRAY, _uiId );
			m_rsCurRenderState.uiTexture2dArray[_ui32Slot] = _uiId;
#ifndef LSA_TOOLS
		}
#endif	// #ifndef LSA_TOOLS
	}
	 ...
	 * \return Returns the OpenGL ID of the texture bound to _ui32Slot.
	 */
	LSE_INLINE GLuint LSE_CALL COpenGl::TextureBinding2dArray( uint32_t _ui32Slot ) {
		assert( _ui32Slot < m_mMetrics.ui32MaxTexSlot );
		return m_rsCurRenderState.uiTexture2dArray[_ui32Slot];
	}
	 ...
	 * \param _bValue The enables the attribute if non-zero, disables it otherwise.
	 */
	LSE_INLINE void LSE_CALL COpenGl::SetVertexAttribArray( GLuint _uiIndex, GLboolean _bValue ) {
	 ...
	 * \param _uiVbo The ID of the vertex buffer to bind.
	 */
	LSE_INLINE void LSE_CALL COpenGl::BindVertexBuffer( GLuint _uiVbo ) {
#ifndef LSA_TOOLS
	 ...
	 * \param _uiIbo The ID of the index buffer to bind.
	 */
	LSE_INLINE void LSE_CALL COpenGl::BindIndexBuffer( GLuint _uiIbo ) {
#ifndef LSA_TOOLS
	 ...
	 * \param _uiFbo The ID of the framebuffer to bind.
	 */
	LSE_INLINE void LSE_CALL COpenGl::BindFramebuffer( GLuint _uiFbo ) {
#ifndef LSA_TOOLS
		if ( m_rsCurRenderState.uiFramebuffer != _uiFbo ) {
#endif	// #ifndef LSA_TOOLS
			m_rsCurRenderState.uiFramebuffer = _uiFbo;
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetSamplerState( uint32_t _ui32Slot, LSBOOL /*_bForce*/ ) {
		glWarnError( "Uncaught" );
...
#undef LSG_CHANGED*/
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetSamplerStates( LSBOOL _bForce ) {
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetRasterizerState( LSBOOL _bForce ) {
		glWarnError( "Uncaught" );
...
#undef LSG_CHANGED
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetRenderTargetBlendState( uint32_t _ui32Slot, LSBOOL _bForce ) {
		glWarnError( "Uncaught" );
...
#define LSG_UPDATE( MEMBER )	rtbsLastState.MEMBER = rtbsCurState.MEMBER
		...
		if ( LSG_CHANGED( BlendEnable ) ) {
			if ( rtbsCurState.BlendEnable ) { glEnablei( GL_BLEND, _ui32Slot ); }
			else { glDisablei( GL_BLEND, _ui32Slot ); }
...
#undef LSG_CHANGED
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetBlendState( LSBOOL _bForce ) {
...
#undef LSG_CHANGED
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetDepthStencilOpState( LSBOOL _bFront, LSBOOL _bForce ) {
		glWarnError( "Uncaught" );
...
#undef LSG_CHANGED
	 ...
	 * \param _bForce Forces changes.
	 */
	LSE_INLINE void LSE_FCALL COpenGl::SetDepthStencilState( LSBOOL _bForce ) {
		glWarnError( "Uncaught" );
		if ( !m_rsCurRenderState.pdssDepthStencilState ) {
			// NULL states are the same as default states.
			m_rsCurRenderState.pdssDepthStencilState = m_pdssDefaultDepthStencilState;
...
#undef LSG_CHANGED
	 ...
	 * Updates vertex attributes for OpneGL.
	 */
	LSE_INLINE void LSE_CALL COpenGl::FinalizeVertAttrib() {
		// Enable and disable delayed vertex attributes.
#ifdef LSA_TOOLS
		for ( GLuint I = LSE_ELEMENTS( m_rsCurRenderState.bVertAttr ); I--; ) {
			if ( m_rsCurRenderState.bVertAttr[I] ) { glEnableVertexAttribArray( I ); }
			else { glDisableVertexAttribArray( I ); }
		}
#else
		for ( GLuint I = CStd::Max( COpenGl::m_rsCurRenderState.uiMaxVertAttrib, COpenGl::m_rsLastRenderState.uiMaxVertAttrib ); I--; ) {
...
#endif	// #ifdef LSA_TOOLS
		m_rsLastRenderState.uiMaxVertAttrib = COpenGl::m_rsCurRenderState.uiMaxVertAttrib;
