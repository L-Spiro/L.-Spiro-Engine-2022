...
#ifndef __LSG_DIRECTX11SHADERMANAGER_H__
#define __LSG_DIRECTX11SHADERMANAGER_H__
...
namespace lsg {
	 ...
	 * \brief Manages the interface between shader objects and the Direct3D 11 API.
	 *
	 * Description: Manages the interface between shader objects and the Direct3D 11 API.  Its main function is to apply
	 *	and unapply shaders, but it also applies engine-specific functionality to shaders as well, where they are
	 *	not supported by HLSL.  For example, automatic application of light and fog values, etc.
	...
	public :
		...
		LSE_CALLCTOR										CDirectX11ShaderManager();
		LSE_CALLCTOR										~CDirectX11ShaderManager();
...
#endif	// __LSG_DIRECTX11SHADERMANAGER_H__
