...
#ifndef __LSG_DIRECTX11SHADER_H__
#define __LSG_DIRECTX11SHADER_H__
...
namespace lsg {
	 ...
	 *	sharing.  Sharing can be disabled for debugging purposes.
	 */
	class CDirectX11Shader : public CShaderBase {
	public :
		// == Various constructors.
		LSE_CALLCTOR										CDirectX11Shader();
		virtual LSE_CALLCTOR								~CDirectX11Shader();
		 ...
		 */
		virtual LSBOOL LSE_CALL								SetFloat( CShaderBase::LSG_HANDLE _hHandle, float _fValue );
		 ...
		 *
		 * \param _hHandle Handle to the value to set in the shader.
		 * \param _piValues New values.
		 * \param _ui32Total Total ints to set.
		 * \return Returns true if the values were set in the shader.
		 */
		virtual LSBOOL LSE_CALL								SetIntArray( LSG_HANDLE _hHandle, const int32_t * _piValues, uint32_t _ui32Total );
		 ...
		 * \return Returns true if the values were set in the shader.
		 */
		virtual LSBOOL LSE_CALL								SetMatrix4x4Array( LSG_HANDLE _hHandle, const CMatrix4x4 * _pmValues, uint32_t _ui32Total );
	...
	protected :
		 ...
		 */
		typedef struct LSG_DX11_SHADER_HANDLE {
			/** The handle for the vertex shader. */
			LSG_HANDLE										hVertexHandle;

			/** The handle for the pixel shader. */
			LSG_HANDLE										hPixelHandle;

			/** The name of the constant associated with this handle. */
			uint32_t										ui32Name;

			/** The shader part that was used to find the given name, even though the name may be in multiple shader parts. */
			LSG_SHADER_TYPES								stType;
		} * LPLSG_DX11_SHADER_HANDLE, * const LPCLSG_DX11_SHADER_HANDLE;
...
#endif	// __LSG_DIRECTX11SHADER_H__
