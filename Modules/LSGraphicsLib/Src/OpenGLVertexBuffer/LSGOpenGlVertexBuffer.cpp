...
#include "LSGOpenGlVertexBuffer.h"
...
namespace lsg {
	...
	COpenGlVertexBuffer::LSG_OPENGL_PREFINED_VERT_ATT COpenGlVertexBuffer::m_opvaVertProps[] = {
		// -- 32-bit Elements. -- //
		// * 1 component. * //
		...
		// -- 16-bit Elements. -- //
		// * 1 component. * //
		{ LSG_VE_R16_F,					2,			1,	GL_HALF_FLOAT,						GL_FALSE },
		...
		// * 2 components. * //
		{ LSG_VE_R16G16_F,				2,			2,	GL_HALF_FLOAT,						GL_FALSE },
		...
		// * 4 components. * //
		{ LSG_VE_R16G16B16A16_F,		2,			4,	GL_HALF_FLOAT,						GL_FALSE },
		...
		// -- 10-bit Elements. -- //
		// * 3 components. * //
		{ LSG_VE_R10G10B10A2_UI,		4,			1,	GL_UNSIGNED_INT_2_10_10_10_REV,		GL_FALSE },
		{ LSG_VE_R10G10B10A2_UI_N,		4,			1,	GL_UNSIGNED_INT_2_10_10_10_REV,		GL_TRUE },
	...
	uint8_t COpenGlVertexBuffer::m_ui8CurVertxAttribEnabled[LSG_MAX_VERTEX_ELEMENT_COUNT] = { false };

	/**< Last enabled vertex attributes. */
	uint8_t COpenGlVertexBuffer::m_ui8LastVertxAttribEnabled[LSG_MAX_VERTEX_ELEMENT_COUNT] = { false };

	// == Various constructors.
	LSE_CALLCTOR COpenGlVertexBuffer::COpenGlVertexBuffer() :
		m_uiVboId( 0 ),
	 ...
	 */
	LSG_VERTEX_BUFFER_MAP LSE_CALL COpenGlVertexBuffer::Map( LSG_VERTEX_BUFFER_MAP_ACCESS _vbmaAccessType ) {
		if ( !m_uiVboId  ) {
		...
		COpenGl::BindVertexBuffer( m_uiVboId );
		LSG_VERTEX_BUFFER_MAP vbmRet = {
			COpenGl::glMapBuffer( GL_ARRAY_BUFFER, _vbmaAccessType & 0xFFFF ),
			Stride() * Elements()
		};
		return vbmRet;
	}
	 ...
	 */
	void LSE_CALL COpenGlVertexBuffer::Unmap( LSG_VERTEX_BUFFER_MAP &_vbmMappedData ) {
		if ( m_uiVboId ) {
			COpenGl::BindVertexBuffer( m_uiVboId );
			COpenGl::glUnmapBuffer( GL_ARRAY_BUFFER );
		}
	 ...
	 */
	LSBOOL LSE_CALL COpenGlVertexBuffer::CreateApiVertexBuffer() {
		ResetApi();
		...
		for ( uint32_t I = 0; I < m_ui16TotalDecl; ++I ) {
			...
			if ( !popvaAttr ) {
				CStd::DebugPrintA( "COpenGlVertexBuffer::CreateApiVertexBuffer(): Unrecognized format in vertex buffer." );
				ResetApi();
				return false;
			}
		...
		// Success.  Upload.
		COpenGl::glBufferData( GL_ARRAY_BUFFER,
			m_ui32Elements * m_ui32Stride,
			m_oobpBuffer.Buffer(),
			m_ui32Usage );

		COpenGl::BindVertexBuffer( 0 );
		if ( ::glGetError() != GL_NO_ERROR ) {
	 ...
	 *	resources.
	 */
	void LSE_CALL COpenGlVertexBuffer::ResetApi() {
		if ( m_uiVboId ) {
			COpenGl::glDeleteBuffers( 1, &m_uiVboId );
			m_uiVboId = 0;
		}
	 ...
	 */
	LSBOOL LSE_CALL COpenGlVertexBuffer::SetStreamApi( uint32_t /*_ui32Index*/, uint32_t _ui32Offset ) const {
	 ...
	 */
	void LSE_CALL COpenGlVertexBuffer::RenderApi( uint32_t _ui32StartVertex, uint32_t _ui32TotalPrimitives ) const {
	 ...
	 */
	void LSE_CALL COpenGlVertexBuffer::PrepareToRenderApi() const {
		assert( CGfx::GetShader() );
		glWarnError( "Uncaught" );
		CCriticalSection::CLockerS lsLockBindBuffer( m_csCrit );
		COpenGl::BindVertexBuffer( m_uiVboId );	// Even if m_uiVboId is 0, that is fine.

		// Set vertex attribute locations.
		for ( uint32_t I = m_ui16TotalDecl; I--; ) {
			const LSG_VERTEX_ATTRIB_POINTER * pvapThis = &m_vapVertexAttribPointer[I];
			if ( pvapThis->ui16LocClass >= CGfx::GetShader()->TotalAttributes() ) { continue; }
			const COpenGlShaderProgramManager::LSG_ATTRIBUTE_USAGE_LOCS * paulLocs = CGfx::GetShader()->GetAttributeLocs();
			GLint iAttrib = paulLocs[pvapThis->ui16LocClass].iLoc[pvapThis->ui16LocUsage];
			if ( iAttrib != -1 ) {
				COpenGl::SetVertexAttribArray( iAttrib, GL_TRUE );
	 ...
	 * Finish rendering.
	 */
	void LSE_CALL COpenGlVertexBuffer::EndRenderApi() const {
		for ( uint32_t I = m_ui16TotalDecl; I--; ) {
			...
			if ( iAttrib != -1 ) {
				COpenGl::SetVertexAttribArray( iAttrib, GL_FALSE );
			}
		}
	}
	 ...
	 */
	const COpenGlVertexBuffer::LSG_OPENGL_PREFINED_VERT_ATT * LSE_CALL COpenGlVertexBuffer::GetVertexElementAttr( LSG_VERTEX_ELEMENT _veType ) {
