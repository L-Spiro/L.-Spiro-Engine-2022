 ...
 *
 *
 * Description: Manages the interface between shader objects and the OpenGL ES API.  Its main function is to apply
...
#ifndef __LSG_OPENGLESSHADERMANAGER_H__
#define __LSG_OPENGLESSHADERMANAGER_H__
...
namespace lsg {
	 ...
	 * \brief Manages the interface between shader objects and the OpenGL ES API.
	 *
	 * Description: Manages the interface between shader objects and the OpenGL ES API.  Its main function is to apply
	 *	and unapply shaders, but it also applies engine-specific functionality to shaders as well, where they are
	 *	not supported by GLSL.  For example, automatic application of light and fog values, etc.
	...
	public :
		...
		LSE_CALLCTOR										COpenGlEsShaderManager();
		LSE_CALLCTOR										~COpenGlEsShaderManager();
...
#endif	// __LSG_OPENGLESSHADERMANAGER_H__
